...
<x-admin-layout>
    ...
    <div class="container-fluid fondo teletrabajo">
        <div class="row">
            <h1 class="text-center font-bold">PERFILES (ROLES)</h1>
                ...
                <div class="col-md-12 p-4 mt-4 bordeRojo bg-blanco-transp rounded-md">
                    <div class="mb-4 sm:flex sm:items-center sm:justify-end">
                        <div class="flex items-center mt-4 gap-x-3">
                            <button
                                ...
                                <a href="{{ route('admin.roles.index') }}" class="">Volver a Perfiles</a>
                            </button>
                        </div>
                    </div>
                    <div class="">
                        <form method="POST" action="{{ route('admin.roles.store') }}">
                            ...
                            <div class="sm:col-span-6 pt-5">
                                <button type="submit"
                                    class="px-4 py-2 bg-red-500 hover:bg-gray-900 hover:text-red-500 rounded-md">Añadir
                                    Perfil</button>
                            </div>
                        </form>
                    </div>
                    <table id="example" class="display" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><span
                                        class="px-3 py-1 text-sm font-bold gap-x-2 rounded-full bg-blue-300">{{ $role->name }}</span>
                                </td>
                                <td>{{ $role->count() }}</td>
                                <td class="px-6 py-4">
                                ...
                                {{-- <td><span class="px-3 py-1 text-sm font-bold gap-x-2 rounded-full bg-{{ $empresa->empresa?->nombre }}">{{ $empresa->empresa?->nombre }}</span>
                                </td>
                                <td><span
                                        class="px-3 py-1 text-sm font-bold gap-x-2 rounded-full bg-{{ $empresa->categoria?->nombre }}">{{ $empresa->categoria?->nombre }}</span>
                                </td> --}}
                            </tr>
                            @endforeach
                        </tbody>
