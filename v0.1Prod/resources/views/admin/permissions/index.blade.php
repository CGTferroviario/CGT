                        ...
                        <td class="px-3 py-1">
                            <div class="flex justify-end gap-4">
                                <a x-data="{ tooltip: 'Edite' }" href="{{ route('admin.permissions.edit', $permission->id) }}"
                                    class="text-green-500 hover:bg-green-500 hover:text-white p-1 rounded-full" title="Editar Permiso">
                                ...
                                <form method="POST" action="{{ route('admin.permissions.destroy', $permission->id) }}" onsubmit="return confirm('¿Seguro que deseas eliminar el permiso?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:bg-red-500 hover:text-black p-1 rounded-full">
                                        <a x-data="{ tooltip: 'Delete' }" class="" href="#" title="Eliminar Permiso">
                                            <svg
