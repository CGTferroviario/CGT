        ...
        <div class="col-md-12">
            <table class="table bg-blanco">        
                ...
                <tr>
                    ...
                    <td>{{ $documento->subtitulo }}</td>
                    <td><span class="badge rounded-pill peq bg-verdeAdif">{{ $documento->empresa }}</span></td>
                    <td><span class="badge rounded-pill peq bg-moradoRenfe">{{ $documento->categoria }}</span></td>
                    <td><span class="badge rounded-pill peq bg-rojoBrillante">{{ $documento->etiqueta }}</span></td>
                    <td>{{ $documento->cuerpo }}</td>
        ...
        <div class="col-xxl-3 col-xl-4 col-md-6 col-sm-12">
            <div class="card h-100 bg-grisOscuro-7 mb-3 blanco bordeRojo" style="">
                <div class="card-header bordeBottomRojo">
                    <a href="{{ route('documentos.show', $documento->id) }}"><h1>{{ $documento->numero }} .- {{ $documento->titulo }}</h1></a>
                    <p class="card-text mt-2">{{ $documento->subtitulo }}</p>
                </div>
                <div class="card-body">
                    <span class="badge rounded-pill peq bg-verdeAdif">{{ $documento->empresa }}</span>
                    <span class="badge rounded-pill peq bg-moradoRenfe">{{ $documento->categoria }}</span>
                    <span class="badge rounded-pill peq bg-rojoBrillante">{{ $documento->etiqueta }}</span>
                    
                    <p class="card-text mt-2 text-justify">{{ $documento->cuerpo }}</p>
                </div>
